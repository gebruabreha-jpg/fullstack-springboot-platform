 Task 2: Node Failure Actions — Complete Code
  ...
  public class K8sNodeSteps implements TestStepDefinition {
      ...
      public void runActionOnNodeTypeNotUsed(final NodeType nodeType, final NodeAction nodeAction) {
       ...
       // Gherkin: Then node "worker-pool1-xxx" status is retrieved
       @TestStep
       @Then("node {string} status is retrieved")
       public void getNodeStatus(final String nodeName) {
           final KubectlApi kubectl = kubectlApiClusterMap.getFirst();
           final String status = kubectl.getNodeStatus(nodeName);
           JcatLoggingApi.setTestInfo("Node '%s' status: %s", nodeName, status);
       }

       // --- STEP 7: Describe node ---
       // Gherkin: When node "worker-pool1-xxx" is described
       @TestStep
       @When("node {string} is described")
       public void describeNode(final String nodeName) {
           final KubectlApi kubectl = kubectlApiClusterMap.getFirst();
           final String describeOutput = kubectl.describeNode(nodeName);
           fetchLogHelper.storeDescribe(nodeName, describeOutput);
           JcatLoggingApi.setTestInfo("Node '%s' describe output collected (%d chars)",
                   nodeName, describeOutput.length());
       }

       // --- STEP 8: Kubelet restart ---
       // Gherkin: When kubelet is restarted on node "worker-pool1-xxx"
       @CriticalTestStep
       @When("kubelet is restarted on node {string}")
       public void restartKubelet(final String nodeName) {
           final K8sNodeApi k8sNodeApi = k8sNodeApisClusterMapProvider.get().getFirst()
                   .getWithHostname(nodeName);
           k8sNodeApi.restartKubelet();
           JcatLoggingApi.setTestInfo("Kubelet restarted on node '%s'", nodeName);
       }

       // --- STEP 9: Kubelet process failure ---
       // Gherkin: When kubelet process is killed on node "worker-pool1-xxx"
       @CriticalTestStep
       @When("kubelet process is killed on node {string}")
       public void killKubeletProcess(final String nodeName) {
           final K8sNodeApi k8sNodeApi = k8sNodeApisClusterMapProvider.get().getFirst()
                   .getWithHostname(nodeName);
           k8sNodeApi.killKubeletProcess();
           JcatLoggingApi.setTestInfo("Kubelet process killed on node '%s'", nodeName);
       }
