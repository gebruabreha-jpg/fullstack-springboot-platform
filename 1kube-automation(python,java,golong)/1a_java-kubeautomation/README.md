...
```com.yourorg.chaos/
 ├── api/        → public APIs (controllers/services)
 ├── chaos/      → chaos actions (core logic)
 ├── health/     → health checks
 ├── helm/       → Helm deployment logic (optional abstraction)
 ├── infra/      → Kubernetes client wrappers
...
Repeated ProvisioningFailed
Example:
Requested 10G, quota is 320G and 319G has been consumed
Requested 4G, quota is 320G and 319G has been consumed
Cause: OpenStack/Cinder quota is full.
Effect: PVCs stay unbound, pods stay pending, StatefulSets/PostgreSQL clusters cannot become ready.

secret:-
Missing secrets/certificates
Many FailedMount events:
secret "eric-sec-sip-tls-trusted-root-cert" not found
secret "eric-pm-server-ca" not found
secret "eric-pc-networking-cm-agent-client-cert" not found
secret "eric-data-object-storage-mn-cert" not found
Cause: expected TLS/cert/config secrets were not generated or not pushed to namespace beets.
Effect: pods start but fail readiness/startup or cannot mount volumes.

Network:-
Service IP / sharing-key conflicts:-
Failed to allocate IP
Example:
address also in use by pcg2/eric-fh-snmp-alarm-provider
port TCP/80 is already in use on "105.2.3.2"
Cause: services in beets are trying to reuse IPs/ports already assigned to services in pcg2.
Effect: those Services may not get stable/expected external IPs.

Node:-
CPU/resource pressure:-
Repeated FailedScheduling
Example:
3 node(s) had untolerated taint(s), 4 Insufficient cpu
Cause: not enough schedulable CPU after taints/resources.
Effect: pods stay pending.

CNI:-
Multus/CNI network setup delays:-
failed to setup network for sandbox
error waiting for pod ... context deadline exceeded
Cause: Multus/Kubernetes API networking delay or overload.
Effect: some pods fail sandbox creation and retry.

kubectl -n beets get pvc
kubectl -n beets get secrets
kubectl describe quota -n beets
kubectl describe resourcequota -n beets
kubectl get nodes -o wide
kubectl describe node <node-name>
kubectl get events -n beets --sort-by=.lastTimestamp